...
function rt(ts){
  ...
  return ts.map(t=>{
    ...
    const ep=(t.endpoints||[]).map(s=>{
      let st
      if(s.status==='ok')st='<span class="ok">✅ OK</span>'
      else if(s.status==='down')st='<span class="fail">❌ '+Number(s.uptime).toFixed(1)+'%</span>'
      else st='<span class="info">⚪ не мониторится</span>'
      return'<span>🌐 Endpoint <span class="v">'+s.period+':</span> '+st+'</span>'
    }).join('')
    const url=t.url?' <span class="info">'+t.url+'</span>':''
    return'<div class="target"><h3>'+t.name+url+'</h3><div class="row">'+cpu+mem+disk+oom+ep+'</div></div>'
  }).join('')
}
...
function rbody(r){
  return rt(r.targets)+rc(r.containers)
}
...
function showReport(i){
  ...
  if(!rs||!rs[i])return
  const r=rs[i]
  $('hd').innerHTML=rbody(r)+'<div class="ts">'+ft(r.timestamp)+' #'+(i+1)+'</div>'
...
async function refresh(){
  ...
  // Dashboard
  $('lr').innerHTML=sr.error?'<div class="empty">'+sr.error+'</div>':rbody(sr)+'<div class="ts">'+ft(sr.timestamp)+'</div>'
